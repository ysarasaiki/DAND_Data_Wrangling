...
API_KEY = { "popular": "6508e453febe414f82669a553259b79b",
            "article": "6508e453febe414f82669a553259b79b"}


def get_from_file(kind, period):
    filename = "popular-{0}-{1}.json".format(kind, period)
    with open(filename, "r") as f:
        return json.loads(f.read()) # encodes json into python


def article_overview_YS(kind, period):
    data = get_from_file(kind, period)
    titles = [] # list of dictionaries, for each article, with {"section":"titles"}
    urls =[] # list of urls that have "format" = "standard thumbnail"
    # YOUR CODE HERE
    # loop through all items
    for entry in data:
        new_article = {}
        new_article[entry["section"]] = entry["title"]
        titles.append(new_article)
        for metadata in entry["media"]: # loops through list of dictionaries
            for media in metadata["media-metadata"]: # looking at list of media dictionaries
                if media["format"] == "Standard Thumbnail":
                    urls.append(media["url"])
    return (titles, urls)

#print article_overview_YS("viewed", 1)

def article_overview_U(kind, period):
    data = get_from_file(kind, period)
    titles = []
    urls =[]
    for article in data:
...
print article_overview_U("viewed", 1)

def query_site(url, target, offset): #, query):
    ...
    if API_KEY["popular"] == "" or API_KEY["article"] == "":
        ...
        return False
    params = {"api-key": API_KEY[target], "offset": offset} #, "q": query}
...
#print query_site(URL_MAIN + 'search/v2/articlesearch.json', "article", 1, "cat")
...
#print get_popular(URL_POPULAR, "viewed", 1)
...
#save_file("viewed", 1)
#print get_from_file("viewed", 1)[1]
